...
window.Cultures = (function () {
  ...
  const generate = function () {
    ...
    function defineCultureType(i) {
      ...
      if (([11, 13, 14, 19, 20, 26, 27, 33, 34].includes(cells.biome[i]) && P(0.7)) || ([28, 35, 40].includes(cells.biome[i]) && P(0.55)) || ([8, 9, 36, 37, 39].includes(cells.biome[i]) && P(0.4))) return "Gnomes"; // no penalty for hills and moutains, high for other elevations
      const f = pack.features[cells.f[cells.haven[i]]]; // opposite feature
      if (f.cells > 5 && P(.075)) return "Amphibians"; 
      if P(.15) return "Dry Reptiles";
